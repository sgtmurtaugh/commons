 ...
 */
class ByteParserTest extends GroovyTestCase {
    void testParse() {
        ByteParser parser = new ByteParser((byte) 125);

        Byte newDefaultValue = 17;
        String sPattern = "#";
        ...
        s = "366,87EUR";
        assertEquals( 110, parser.parse(s) );
        assertEquals( 110, parser.parse(s, newDefaultValue) );
        assertEquals( 110, parser.parse(s, newDefaultValue, sPattern) );
        assertEquals( 110, parser.parse(s, newDefaultValue, sPattern, decimalFormatSymbols ) );
        ...
        s = "6.005,3";
        assertEquals( 117, parser.parse(s) );
        assertEquals( 117, parser.parse(s, newDefaultValue) );
        ...
        s = "423,005.3";
        assertEquals( -89, parser.parse(s) );
        assertEquals( -89, parser.parse(s, newDefaultValue) );
        assertEquals( -89, parser.parse(s, newDefaultValue, sPattern) );
        assertEquals( -89, parser.parse(s, newDefaultValue, sPattern, decimalFormatSymbols ) );
