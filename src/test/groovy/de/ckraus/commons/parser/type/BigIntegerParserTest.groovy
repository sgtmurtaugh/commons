 ...
 */
class BigIntegerParserTest extends GroovyTestCase {
    void testParse() {
        BigIntegerParser parser = new BigIntegerParser( new BigInteger(963852) );

        BigInteger newDefaultValue = new BigInteger(741852);
        String sPattern = "#";
        ...
        s = "41,54€";
        assertEquals( 41, parser.parse(s) );
        assertEquals( 41, parser.parse(s, newDefaultValue) );
        assertEquals( 41, parser.parse(s, newDefaultValue, sPattern) );
        assertEquals( 41, parser.parse(s, newDefaultValue, sPattern, decimalFormatSymbols ) );

        s = "366,87EUR";
        assertEquals( 366, parser.parse(s) );
        assertEquals( 366, parser.parse(s, newDefaultValue) );
        assertEquals( 366, parser.parse(s, newDefaultValue, sPattern) );
        assertEquals( 366, parser.parse(s, newDefaultValue, sPattern, decimalFormatSymbols ) );
        ...
        s = "6.005,3";
        assertEquals( 6005, parser.parse(s) );
        assertEquals( 6005, parser.parse(s, newDefaultValue) );
        ...
        s = "423,005.3";
        assertEquals( 423, parser.parse(s) );
        assertEquals( 423, parser.parse(s, newDefaultValue) );
        assertEquals( 423, parser.parse(s, newDefaultValue, sPattern) );
        assertEquals( 423, parser.parse(s, newDefaultValue, sPattern, decimalFormatSymbols ) );
