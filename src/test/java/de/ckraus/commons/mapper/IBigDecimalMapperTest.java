...
class IBigDecimalMapperTest {
    ...
    void unformat() {
        ...
        BigDecimal defaultBigDecimalValue   = (BigDecimal) getParameters().get(PARAMS_KEY__BIG_DECIMAL);
        BigDecimal defaultBigIntegerValue   = BigDecimal.valueOf(
                ( (BigInteger) getParameters().get(PARAMS_KEY__BIG_INTEGER) ).doubleValue()
        );
        BigDecimal defaultByteValue         = BigDecimal.valueOf(
                ( (Byte) getParameters().get(PARAMS_KEY__BYTE) ).doubleValue()
        );
        BigDecimal defaultDoubleValue       = BigDecimal.valueOf(
                ( (Double) getParameters().get(PARAMS_KEY__DOUBLE) )
        );
        BigDecimal defaultFloatValue        = BigDecimal.valueOf(
                ( (Float) getParameters().get(PARAMS_KEY__FLOAT) )
        );
        BigDecimal defaultIntegerValue      = BigDecimal.valueOf(
                ( (Integer) getParameters().get(PARAMS_KEY__INTEGER) ).doubleValue()
        );
        BigDecimal defaultLongValue         = BigDecimal.valueOf(
                ( (Long) getParameters().get(PARAMS_KEY__LONG) ).doubleValue()
        );
        BigDecimal defaultShortValue        = BigDecimal.valueOf(
                ( (Short) getParameters().get(PARAMS_KEY__SHORT) ).doubleValue()
        );

        assertEquals( defaultBigDecimalValue, mapper.unformat( sBigDecimalString ) );
        assertEquals( defaultBigIntegerValue, mapper.unformat( sBigIntegerString ) );
        assertEquals( defaultByteValue, mapper.unformat( sByteString ) );
        assertEquals( defaultDoubleValue, mapper.unformat( sDoubleString ) );
        assertEquals( defaultFloatValue, mapper.unformat( sFloatString ) );
        assertEquals( defaultIntegerValue, mapper.unformat( sIntegerString ) );
        assertEquals( defaultLongValue, mapper.unformat( sLongString ) );
        assertEquals( defaultShortValue, mapper.unformat( sShortString ) );
    ...
    /**
     * test for {@link IBigDecimalMapper#unformatToNumber(String, Locale, String, DecimalFormatSymbols, Number)}
     */
    @Test
